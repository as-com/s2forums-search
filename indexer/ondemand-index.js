...
function parsePost(response, body, id) {
	...
	var time = moment(Date.create($(p + ".box-head > a").text()).getTime()).tz("UTC").format("YYYY-MM-DDTHH:MM:SS");
	...
	try {
		...
		postEditTime = moment(Date.create($(p + ".posteditmessage").text().split("Last edited by ")[1].split(" (")[1].split(")")[0]).getTime()).tz("UTC").format("YYYY-MM-DDTHH:MM:SS");
